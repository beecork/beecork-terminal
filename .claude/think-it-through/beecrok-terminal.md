...
- **Session busy/idle dot & agent-name label** — **RESOLVED and agent-agnostic**, though *not* the way this brief first assumed. As shipped, the busy dot is driven by **webview output activity** (`useSessionStatus.ts`: output pausing for `QUIET_MS` turns it off), **not** by the tty's foreground process group. The foreground check in `pty.rs` survives for *cosmetic labelling only* — a small `KNOWN_TOOLS` list prettifies the session title and feeds the "come look" path; unknown tools fall back to their own command name, so nothing depends on it. Output-activity also works on Windows, where `tcgetpgrp` has no equivalent. See the comment at `pty.rs` above `KNOWN_TOOLS`.
